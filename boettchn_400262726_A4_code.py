...
def forward_propagation_test(x_train, weights_layer_1, weights_layer_2, weights_output):
    ...
    z_3, h_3 = process_layer(output_layer_2, weights_output, False) #z_3 is before sigmoid, h_3 is after sigmoid - h_3 = sigmoid(z_3)
...
def calculate_loss(x_set, t_set, weights_layer_1, weights_layer_2, weights_output):
    ...
    for index in range(len(x_set)):
        ...
        sigmoid_value = expit(pre_sigmoid[0])

        cross_entro_loss += float(t_set[index]) * np.log(sigmoid_value) + (1 - float(t_set[index])) * np.log(1 - sigmoid_value)

        if output_class == int(t_set[index]): 
            accuracy += 1

    average_cross_entro_loss = -cross_entro_loss / len(x_set)  # Negative sign to convert to average and handle minimization
    print("Average Cross-Entropy Loss:", average_cross_entro_loss)
    print("Accuracy:", accuracy / len(x_set))

    return average_cross_entro_loss
...
# def cross_entro_loss(t_set_element, pre_sigmoid):
#     return float(t_set_element)*np.logaddexp(0,-float(pre_sigmoid[0])) + (1-float(t_set_element))*np.logaddexp(0,float(pre_sigmoid[0]))
...
#     pre_sigmoid, z_3, output_class = forward_propagation_test(x_set, weights_layer_1, weights_layer_2, weights_output)  #act the forward propagation on the whole x_set
#     cross_entro_loss = cross_entro_loss(t_set, pre_sigmoid)

#     return np.average(cross_entro_loss)
...
def main():
    ...
    plt.suptitle("Training and Validation cross entropy loss per Epoch - (3,2)")
    plt.show()
